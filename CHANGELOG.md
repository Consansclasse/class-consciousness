...
- ADRs : 0001 stack, 0002 TEI, 0003 RAG, 0004 modèle données, 0005 ARK, **0006 déploiement Coolify sur OVH `51.68.129.187` + `consciencedeclasse.com`** (multi-apps, critère SSD obligatoire), **0007 analytique Matomo cookieless** (mode CNIL strict, sous-domaine `matomo.consciencedeclasse.com`)
...
- Compose Matomo (matomo apache + mariadb LTS + cron archivage horaire) déployé via Coolify Docker Compose Empty — pas dans le repo (cf. `ops/runbooks/matomo-deploy.md`)
